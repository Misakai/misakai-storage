...
namespace Misakai.Storage
{
    ...
    public class DbContextCollection : IDbContextCollection
    {
        ...
        public DbContext Get(Type requestedType)
        {
            if (_disposed)
                throw new ObjectDisposedException("DbContextCollection");

            if (requestedType.IsSubclassOf(typeof(DbContext)))
                throw new ArgumentException("requestedType should be a subclass of DbContext type.");

            if (!_initializedDbContexts.ContainsKey(requestedType))
            {
                var method = _dbContextFactory.GetType().GetMethod("CreateDbContext");
                var generic = method.MakeGenericMethod(requestedType);

                // First time we've been asked for this particular DbContext type.
                // Create one, cache it and start its database transaction if needed.
                var dbContext = _dbContextFactory != null
                    ? generic.Invoke(this, null) as DbContext
